...
std::string Scheme::Evaluate(const std::string& expression) {
    ...
    return ObjectToString(ans);
}

llvm::Value* Scheme::Codegen(const std::string& expression, std::shared_ptr<llvm::Module> module, llvm::IRBuilder<>& builder) {
    std::stringstream ss{expression};
    Tokenizer tokenizer{&ss};
    std::shared_ptr<Object> current = Read(&tokenizer);
    ...
    current->Codegen(module, builder, object_type, {}, global_scope_);
    return nullptr;
}
